...
@dataclass
class AudibleProductScraperConfig(ScrapedModelConfig):
...
class AudibleProductScraperData(TypedDict, total=False):
    title: str | None
...
class AudibleProductScraper(ScrapedModel):
    config: AudibleProductScraperConfig = AudibleProductScraperConfig()
    _URL_RE = re.compile(r"audible\.([a-z.]+)/(?:pd|podcast|ac)/[^/]*/?(B0[A-Z0-9]{8}|\d{10})(?:[?#]|$)", re.IGNORECASE)

    @staticmethod
    def is_audible_url(url: str) -> bool:
        return bool(AudibleProductScraper._URL_RE.search(url))

    @staticmethod
    def parse_url(url: str) -> ProductInput | None:
        m = AudibleProductScraper._URL_RE.search(url)
    ...
    def __init__(
        ...
        if url:
            parsed = AudibleProductScraper.parse_url(url)
            if not parsed:
    ...
    def __str__(self) -> str:
        return f"AudibleProductScraper({self.tld}, {self.asin})"
    ...
    @classmethod
    def _from_input(cls, item: ProductInput) -> "AudibleProductScraper":
        tld, asin = item
        return cls(tld, asin)
    ...
    async def _parse_html(self, html: str, scraper=None) -> AudibleProductScraperData:
    ...
    def scrape_stream(
        ...
        upload_images: bool = True,
        clear_cache: bool = False,
    ) -> AsyncGenerator["AudibleProductScraper", None]:
